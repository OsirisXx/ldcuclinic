-- =====================================================
-- SAFE CREATE DOCTORS AND NURSES
-- Only inserts profiles for users that EXIST in auth.users
-- =====================================================
...
-- Step 2: Create profiles ONLY for users that exist in auth.users
DO $$
DECLARE
    v_campus_id UUID;
    v_dept_id UUID;
BEGIN
    ...
    -- Doctor 1: doctor@test.com (e0307373-185a-4c8e-acdc-86c02e00ca65)
    IF EXISTS (SELECT 1 FROM auth.users WHERE id = 'e0307373-185a-4c8e-acdc-86c02e00ca65') THEN
        ...
        RAISE NOTICE 'Created profile for doctor@test.com';
    END IF;

    -- Doctor 2: doctor1@test.com (b57727d0-9e8e-44bd-9d1b-1e3f5e14ccff)
    IF EXISTS (SELECT 1 FROM auth.users WHERE id = 'b57727d0-9e8e-44bd-9d1b-1e3f5e14ccff') THEN
        INSERT INTO profiles (id, email, first_name, last_name, role, department_id, campus_id, is_verified, auth_provider)
        ...
        RAISE NOTICE 'Created profile for doctor1@test.com';
    END IF;

    -- Doctor 3: doctor2@test.com (a3658cdd-5b25-4ae8-b71b-9c607a6bb225)
    IF EXISTS (SELECT 1 FROM auth.users WHERE id = 'a3658cdd-5b25-4ae8-b71b-9c607a6bb225') THEN
        INSERT INTO profiles (id, email, first_name, last_name, role, department_id, campus_id, is_verified, auth_provider)
        ...
        RAISE NOTICE 'Created profile for doctor2@test.com';
    END IF;

    -- Doctor 4: doctor3@test.com (6f260830-1d18-450f-85ae-2b58bcd7d019)
    IF EXISTS (SELECT 1 FROM auth.users WHERE id = '6f260830-1d18-450f-85ae-2b58bcd7d019') THEN
        INSERT INTO profiles (id, email, first_name, last_name, role, department_id, campus_id, is_verified, auth_provider)
        ...
        RAISE NOTICE 'Created profile for doctor3@test.com';
    END IF;

    -- Doctor 5: doctor4@test.com (38bee1cd-8e7a-4396-8ef1-87d2b81af723)
    IF EXISTS (SELECT 1 FROM auth.users WHERE id = '38bee1cd-8e7a-4396-8ef1-87d2b81af723') THEN
        INSERT INTO profiles (id, email, first_name, last_name, role, department_id, campus_id, is_verified, auth_provider)
        ...
        RAISE NOTICE 'Created profile for doctor4@test.com';
    END IF;

    -- Employee: employee@test.com (4fbe0a79-4b1f-42e9-88c3-074dcc2884f5)
    IF EXISTS (SELECT 1 FROM auth.users WHERE id = '4fbe0a79-4b1f-42e9-88c3-074dcc2884f5') THEN
        INSERT INTO profiles (id, email, first_name, last_name, role, department_id, campus_id, is_verified, auth_provider)
        ...
        RAISE NOTICE 'Created profile for employee@test.com';
    END IF;

    -- Employee1: employee1@test.com (21f562a9-caac-4846-816d-1fe9b5a92cbf)
    IF EXISTS (SELECT 1 FROM auth.users WHERE id = '21f562a9-caac-4846-816d-1fe9b5a92cbf') THEN
        INSERT INTO profiles (id, email, first_name, last_name, role, department_id, campus_id, is_verified, auth_provider)
        ...
        RAISE NOTICE 'Created profile for employee1@test.com';
    END IF;

    -- Employee2: employee2@test.com (90487343-e474-4a1f-9e9f-1f7fb141f25f)
    IF EXISTS (SELECT 1 FROM auth.users WHERE id = '90487343-e474-4a1f-9e9f-1f7fb141f25f') THEN
        INSERT INTO profiles (id, email, first_name, last_name, role, department_id, campus_id, is_verified, auth_provider)
        ...
        RAISE NOTICE 'Created profile for employee2@test.com';
    END IF;

    -- Nurse: nurse@test.com (2485d3cd-c05e-4c1f-bd2e-4dabb300fda4)
    IF EXISTS (SELECT 1 FROM auth.users WHERE id = '2485d3cd-c05e-4c1f-bd2e-4dabb300fda4') THEN
        INSERT INTO profiles (id, email, first_name, last_name, role, department_id, campus_id, is_verified, auth_provider)
        ...
        RAISE NOTICE 'Created profile for nurse@test.com';
    END IF;

    -- Nurse1: nurse1@test.com (6808c505-1cb5-43cc-990b-fcb233c82d6c)
    IF EXISTS (SELECT 1 FROM auth.users WHERE id = '6808c505-1cb5-43cc-990b-fcb233c82d6c') THEN
        INSERT INTO profiles (id, email, first_name, last_name, role, department_id, campus_id, is_verified, auth_provider)
        ...
        RAISE NOTICE 'Created profile for nurse1@test.com';
    END IF;

END $$;

-- Step 3: Add to doctors table (only if profile exists)
DO $$
DECLARE
    v_campus_id UUID;
BEGIN
    ...
    SELECT id, 'General Medicine', 'PRC-MD-2001-001', v_campus_id, true
    FROM profiles WHERE id = 'e0307373-185a-4c8e-acdc-86c02e00ca65' AND role = 'doctor'
    ON CONFLICT (profile_id) DO UPDATE SET specialization = EXCLUDED.specialization;

    INSERT INTO doctors (profile_id, specialization, license_number, campus_id, is_active)
    SELECT id, 'Internal Medicine', 'PRC-MD-2002-002', v_campus_id, true
    FROM profiles WHERE id = 'b57727d0-9e8e-44bd-9d1b-1e3f5e14ccff' AND role = 'doctor'
    ON CONFLICT (profile_id) DO UPDATE SET specialization = EXCLUDED.specialization;

    INSERT INTO doctors (profile_id, specialization, license_number, campus_id, is_active)
    SELECT id, 'Pediatrics', 'PRC-MD-2003-003', v_campus_id, true
    FROM profiles WHERE id = 'a3658cdd-5b25-4ae8-b71b-9c607a6bb225' AND role = 'doctor'
    ON CONFLICT (profile_id) DO UPDATE SET specialization = EXCLUDED.specialization;

    INSERT INTO doctors (profile_id, specialization, license_number, campus_id, is_active)
    SELECT id, 'Family Medicine', 'PRC-MD-2004-004', v_campus_id, true
    FROM profiles WHERE id = '6f260830-1d18-450f-85ae-2b58bcd7d019' AND role = 'doctor'
    ON CONFLICT (profile_id) DO UPDATE SET specialization = EXCLUDED.specialization;

    INSERT INTO doctors (profile_id, specialization, license_number, campus_id, is_active)
    SELECT id, 'Occupational Medicine', 'PRC-MD-2005-005', v_campus_id, true
    FROM profiles WHERE id = '38bee1cd-8e7a-4396-8ef1-87d2b81af723' AND role = 'doctor'
    ON CONFLICT (profile_id) DO UPDATE SET specialization = EXCLUDED.specialization;
END $$;

-- Step 4: Add to nurses table (only if profile exists)
DO $$
DECLARE
    v_campus_id UUID;
BEGIN
    ...
    SELECT id, 'PRC-RN-2010-001', v_campus_id, true
    FROM profiles WHERE id = '2485d3cd-c05e-4c1f-bd2e-4dabb300fda4' AND role = 'nurse'
    ON CONFLICT (profile_id) DO UPDATE SET license_number = EXCLUDED.license_number;

    INSERT INTO nurses (profile_id, license_number, campus_id, is_active)
    SELECT id, 'PRC-RN-2012-002', v_campus_id, true
    FROM profiles WHERE id = '6808c505-1cb5-43cc-990b-fcb233c82d6c' AND role = 'nurse'
    ON CONFLICT (profile_id) DO UPDATE SET license_number = EXCLUDED.license_number;
END $$;
